...
class TemplateMinerConfig:
    ...
    def load(self, config_filename: str):
        ...
        self.parametrize_numeric_tokens = parser.getboolean(
            section_drain,
            "parametrize_numeric_tokens",
            fallback=self.parametrize_numeric_tokens,
        )

        masking_instructions_str = parser.get(
            section_masking, "masking", fallback=str(self.masking_instructions)
        )
        self.mask_prefix = parser.get(
            section_masking, "mask_prefix", fallback=self.mask_prefix
        )
        self.mask_suffix = parser.get(
            section_masking, "mask_suffix", fallback=self.mask_suffix
        )
        self.parameter_extraction_cache_capacity = parser.get(
            section_masking,
